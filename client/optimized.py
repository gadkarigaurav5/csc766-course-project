...
def main():
    ...
    parser_payload = {"source_code": source_code,"include_extras": False}
    t0 = pu.now_ms()
    parser_output = requests.post(PARSER_URL, json=parser_payload).json()
    t1 = pu.now_ms()
    rpc_calls += 1
    parser_record = pu.build_exec_op_record(
        trace_id=trace_id,
        node_id="parser_1",
        op="tool.parser",
        t_start_ms=t0,
        t_end_ms=t1,
        payload_in_bytes=pu.guess_bytes(parser_payload),
        payload_out_bytes=pu.guess_bytes(parser_output),
        inputs_meta={"source_code": make_meta(source_code, "str"),"include_extras": make_meta(False, "bool")},
        
        outputs_meta={ "functions":make_meta(parser_output.get("functions", []), "json"),"classes": make_meta(parser_output.get("classes", []), "json")}
        
    )
    pu.append_jsonl(LOG_PATH, parser_record)
    
    
    
    
    lint_payload = {"functions":parser_output.get("functions", []),"classes":parser_output.get("classes", [])}
    t2= pu.now_ms()
    ...
    inputs_meta={"functions": make_meta(lint_payload["functions"], "json"),"classes" :make_meta(lint_payload["classes"], "json")},
    ...
    doc_payload = {"functions": parser_output.get("functions", []), "classes": parser_output.get("classes", [])}
    t4 = pu.now_ms()
